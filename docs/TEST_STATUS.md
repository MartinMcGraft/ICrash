...
- `flutter test --no-pub`: passed, **83 tests** as of this round (up from 69, up from 48 before that) — `test/domain/inventory_rules_test.dart` (11, including the two literal critical inventory scenarios from spec sections 58 and 59), `test/services/gs1_data_matrix_parser_test.dart` (10, new: pure GS1 AI parsing — FNC1 handling, out-of-order AIs, the documented no-separator truncation limitation, year-window decoding, malformed input), `test/widget_test.dart` (legacy `HomeMenu` smoke test, now pumped directly rather than via `MyApp` since `MyApp` requires a real Firebase app), and presentation-layer suites using fakes from `test/fakes/fake_repositories.dart`: `login_screen_test.dart` (3), `institution_selection_screen_test.dart` (3), `institution_home_screen_test.dart` (11, cart list/empty-state/creation/role-gated FAB/Membros-icon-gating/legacy-access/status-counts/name-filter/history-action/recent-activity), `members_screen_test.dart` (5, empty state/list/create/self-row-hides-menu/disable), `cart_detail_screen_test.dart` (11, empty state/drawer list/role-gated creation/bumps-layout-version/Responsáveis-shown-hidden-by-role/edit/duplicate-without-stock/edit-duplicate-hidden-for-normal-user/search-action-shown), `slot_editor_screen_test.dart` (12, unit grid render/merge/split/view-only-for-normal-user/assign-a-product/record-consumption/replenish-via-GS1-scan/correct-an-event/reconcile-after-audit/role-gating-for-a-normal-user/informational-message-for-a-normal-user/refuses-duplicate-product-assignment), `products_screen_test.dart` (4, empty state/list/role-gated creation), `responsible_users_screen_test.dart` (3), `product_search_screen_test.dart` (3), `history_screen_test.dart` (3), `gs1_scan_screen_test.dart` (3, new: parses-and-pops/shows-retry-on-unrecognized-payload/cancel-returns-null).
