...
- `flutter test --no-pub`: passed, **108 tests** as of this round (up from 83, 69, 48 before that) — `test/domain/inventory_rules_test.dart` (19: 11 conservative-expiry/lot tests including the two literal critical inventory scenarios from spec sections 58-59, plus 8 for `InventoryRules.computeAlert`'s cross-cart alert logic), `test/services/gs1_data_matrix_parser_test.dart` (10, pure GS1 AI parsing), `test/services/internal_qr_payload_test.dart` (5, new: encode/decode round trip, wrong scheme/version/resource-kind, malformed input), `test/services/gs1_hid_scanner_service_test.dart` (2, new: emits a fed line, ignores an empty one), `test/widget_test.dart` (legacy `HomeMenu` smoke test), and presentation-layer suites using fakes from `test/fakes/fake_repositories.dart`: `login_screen_test.dart` (3), `institution_selection_screen_test.dart` (3), `institution_home_screen_test.dart` (16: the 11 from before plus scan-cart-QR-opens-it/scan-error-when-not-found/shows-cross-cart-alerts-for-manager/hides-alerts-for-normal-user/no-alerts-when-nothing-needs-attention), `members_screen_test.dart` (5), `cart_detail_screen_test.dart` (12: the 11 from before plus shows-cart-QR-code), `slot_editor_screen_test.dart` (12), `products_screen_test.dart` (4), `responsible_users_screen_test.dart` (3), `product_search_screen_test.dart` (3), `history_screen_test.dart` (3), `gs1_scan_screen_test.dart` (4: the 3 from before plus the HID text-field flow), `cart_qr_scan_screen_test.dart` (3, new: decodes-and-pops/retry-on-unrecognized-payload/cancel-returns-null).
