...
One gap flagged at the time as deliberately *not* fixed — `SlotEditorScreen`'s grid cells could shrink below the 48×48dp minimum touch target on a densely-configured drawer — was revisited and fixed in the "reporting, orphaned-slot cleanup, touch-target fix" pass below.
...
## Phase 2 continued — reporting aggregates/PDF export, orphaned-slot cleanup, slot grid touch-target fix

Status: complete and validated live on the Android emulator.

- `lib/src/presentation/reports/history_summary.dart` gained `summarizeByCart`/`summarizeByPeriod` (day/week/Monday-start-week/month bucketing) alongside the existing `summarizeByProduct`, closing spec section 51's per-cart/per-period aggregate gap. `HistoryScreen`'s "Ver resumo" dialog became `_SummaryDialog`, a `SegmentedButton`-switchable view across all three dimensions instead of a single fixed product breakdown.
- New `lib/src/presentation/reports/history_pdf_export.dart` (`pdf` + `printing` packages — pure Dart PDF generation, no platform channel to build; `printing` hands the bytes to the OS's own print/share sheet, so the app never writes a file or requests filesystem access itself) alongside the existing CSV export, closing spec section 51's PDF-export gap.
- `SlotEditorScreen` now detects a `CartProductAssignment` whose `slotId` no longer matches any slot in *any* drawer of the cart (left behind by a past merge/split — the id it points to no longer exists anywhere, not just in this drawer, since ids are only unique within their own drawer's grid) and shows a manager-only banner offering to reassign it to a currently-empty slot in this drawer or delete it outright. New `InventoryRepository.reassignSlot`/`deleteAssignment` methods back this (both simple Firestore operations following the same patterns as the repository's existing methods; no `firestore.rules` changes were needed — the existing manager+ `update`/`delete` rules on `assignments` already covered both).
- The slot grid's cell size is now floored at 48 logical pixels (WCAG/Material's minimum touch target) — a dense drawer whose ideal proportional cell size would fall below that now scrolls (two nested `SingleChildScrollView`s) instead of shrinking cells further, closing the touch-target gap the accessibility review had flagged and deliberately deferred.
- New/extended tests: 6 in `history_summary_test.dart` (per-cart, per-cart-missing-name, per-period × day/week/month, skips-unsynced-events), 3 in `history_screen_test.dart` (per-cart summary, per-period summary, PDF export button present), 4 in `slot_editor_screen_test.dart` (flags-and-deletes an orphan, flags-and-reassigns an orphan, dense-drawer scrolls). Total: 131/131 passing, `flutter analyze --no-pub` clean.
- Live Android validation: per-cart and per-period summary groupings both showed correct real data; "Exportar PDF" opened the platform print/share sheet with a rendered one-page PDF (title in the current UI language, table headers/data in fixed Portuguese — see the localization entry below for why); the slot grid rendered and behaved normally on a 5×7 drawer. No fatal exceptions in logcat.

## Phase 2 continued — PT-PT/English localization (spec section 13)

Status: complete and validated live on the Android emulator — the last still-open item from spec section 13, previously not started at all.

- Added `flutter_localizations` + `flutter gen-l10n` codegen (`lib/l10n/app_pt.arb`/`app_en.arb` → `lib/src/common/l10n/app_localizations*.dart`, configured via `l10n.yaml` with an explicit non-synthetic output directory). Every screen's hardcoded PT-PT string — roughly 250 across 23 files — now resolves through `AppLocalizations.of(context)` instead.
- New `lib/src/common/locale_scope.dart` (`LocaleScope`, a plain `InheritedWidget` mirroring `AppServicesScope`'s own DI approach) and a rewritten `main.dart`: `MyApp` is now a `StatefulWidget` holding the current `Locale`, defaulting to PT-PT regardless of the device's own system locale (this app is deployed in Portugal first) and persisting a user's explicit choice via `shared_preferences`. A language-switcher icon (`InstitutionHomeScreen`'s `AppBar`) lets any user flip between Português/English at runtime.
- The five cross-cutting label helpers (`assignmentStatusLabel`, `usageEventTypeLabel`, `cartStatusLabel`, `roleLabel`, `membershipStatusLabel`) now take a `BuildContext` and resolve through `AppLocalizations` for on-screen (widget) use. **Deliberate design decision**: the CSV/PDF export builders (`history_csv_export.dart`/`history_pdf_export.dart`) keep their own fixed-Portuguese variants (`assignmentStatusLabelPt`/`usageEventTypeLabelPt`) and stay independent of the current UI locale — a data-interchange export format changing depending on who happened to export it seemed worse than keeping it fixed; see `docs/ARCHITECTURE.md`'s "Export language" note. `summarizeByProduct`/`summarizeByCart` gained an optional `removedLabel` parameter (defaulting to the old fixed-PT string) so the on-screen summary dialog can localize its "removed" placeholder while the pure functions themselves stay `BuildContext`-free and unit-testable.
- Every widget test's `MaterialApp` now wires `locale: const Locale('pt', 'PT')` plus `AppLocalizations`'s delegates/supported-locales, so existing PT-PT text assertions keep matching (Flutter's test binding otherwise defaults to `en_US`, which would have silently broken every existing assertion). A new end-to-end test (`institution_home_screen_test.dart`) drives the actual language switcher through a `LocaleScope`-aware harness and asserts the screen's rendered text changes to English, not just that the code compiles.
- Total: 132/132 tests passing, `flutter analyze --no-pub` clean.
- Live Android validation: switched the running app from Português to English via the AppBar globe icon — every screen visited (dashboard, history, cart detail, reporting) re-rendered instantly in English, including cart/product *names* correctly staying as entered (only interface chrome translates); a PDF exported while in English carried an English title with the export's own data table still in fixed Portuguese, exactly as designed. No fatal exceptions in logcat across the whole session (one transient ANR on first cold launch, traced to a whole-device CPU/memory pressure spike unrelated to app code — see `docs/TEST_STATUS.md`).

This closes every workstream in `ICRASH_V2_SPECIFICATION.md` that an assistant can complete without human account access, real secrets, or an invented clinical/business decision. See `docs/AI_HANDOFF.md`'s "Full remaining punch-list" for what is left and why.
