...
- Last published commit before this continuation: `9ad6596` (`docs: record responsibleUsers/cart edit-duplicate/product search/dashboard/history status, live test results and handoff`)
...
Phase 0 (Git/modernization) and Phase 1 (Firebase foundation) are complete and published. Phase 2's architecture foundation (layered domain/data/repositories, Firestore model, security Rules with tests) is complete. **This continuation session added an eighth round of V2 work**: two correctness fixes (product uniqueness per cart, layout versioning), five more presentation slices ((1) `responsibleUsers` management, (2) cart edit/duplicate, (3) product search within a cart, (4) a real institution dashboard, (5) an institution-wide activity history screen), and finally **GS1 Data Matrix scanning** (spec sections 29-32) — all validated live end-to-end on the Android emulator, with zero new app bugs found. `flutter test` is now 83/83. **Every major workstream in the V2 specification's core scope is now implemented.** The only work explicitly deferred throughout this whole effort is documented in "What is intentionally not built yet" in `docs/ARCHITECTURE.md` — internal QR/cart navigation, reporting export/aggregates, and cross-cart dashboard alerts.

### GS1 Data Matrix scanning sub-phase (most recent, and the highest-priority item per explicit user instruction)
...
GS1 Data Matrix scanning (this sub-phase, most recent):
...
`flutter analyze` clean. `flutter test`: 83/83 (up from 69, up from 48 before that). `flutter build apk --debug`: passed. Firestore Rules tests: 22/22 (unchanged this round — no Rules changes). Eight full live Android walkthroughs: login/institution-selection, cart list/creation, membership management, drawer/slot editor, product catalogue/slot assignments, audit reconciliation/correction, correctness-fixes/responsibleUsers/cart-edit-duplicate/product-search/dashboard/history, then GS1 Data Matrix scanning — see `docs/TEST_STATUS.md` for detail (zero new app bugs across the last two rounds; the AVD's virtual camera cannot produce a real scannable code, so the GS1 decode-and-prefill path was validated via an automated widget test with a fake scanner instead, per spec section 5's own prescribed strategy).
...
**Every major workstream from the V2 specification's core scope is now implemented**: login, institution selection, cart list/creation/edit/duplicate, membership management, `responsibleUsers` management, the drawer/slot editor, product catalogue/slot assignments, audit reconciliation/correction, product search, the dashboard, the activity history screen, and GS1 Data Matrix scanning for replenishment. `InventoryRepository`'s entire surface is reachable from the UI, both correctness gaps found in spec review (product uniqueness, layout versioning) are fixed, and `ProductRepository.findByGtin`/`Batch.gtin`/`Batch.source` — previously unused — are now all exercised by the UI.

There is no single next task handed down by the user at this point; whoever picks this up next should ask what to prioritize among the remaining, explicitly-deferred items (see "What is intentionally not built yet" in `docs/ARCHITECTURE.md`), roughly in order of how much of the specification's core intent they still leave open:

1. **Internal QR-driven cart/drawer navigation** (spec section 33): `InternalQrScannerService` is still a contract only. The legacy `lib/qr_code_reader/qr_code_reader.dart` already demonstrates scanning an internal QR payload with `mobile_scanner` (though wired to the legacy `RequestHandler`, not Firestore) — same reuse-the-package-not-the-widget pattern as the GS1 scanner just built would apply here.
2. **Reporting export and aggregates** (the rest of spec section 51): PDF/CSV export and per-product/per-cart/per-period aggregate reports, beyond the read-only filterable history view already built.
3. **Cross-cart dashboard alerts** (the rest of spec section 49): expiring/expired products across every cart in an institution, which needs an institution-wide `assignments` collectionGroup query and a matching Rules change — deliberately deferred twice now given the `memberIndex` collectionGroup/Rules quirks already documented in this file and `docs/FIREBASE_MODEL.md`; read that history before attempting it.
4. Anything from `docs/OPEN_DECISIONS.md` the user has since resolved.

Whichever is picked, follow the same pattern established throughout: check `lib/src/domain/repositories/`/`lib/src/services/` first for what already exists, fakes go in `test/fakes/`, screens go in `lib/src/presentation/<area>/`, and validate live on the Android emulator before calling it done — not just `flutter analyze`/`flutter test`. Also visually re-check for control overlap since widget tests don't catch that — see the cart-list sub-phase's bug above.
