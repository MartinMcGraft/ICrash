...
void main() {
  ...
  testWidgets('submits entered credentials to AuthRepository', (tester) async {
    ...
    expect(find.text('Não foi possível iniciar sessão. Tente novamente.'), findsNothing);
  });

  testWidgets('shows a friendly message on invalid credentials', (tester) async {
    final auth = FakeAuthRepository(signInError: const RepositoryFailure(RepositoryFailureReason.unauthenticated));
    await tester.pumpWidget(_wrap(buildTestServices(auth: auth)));
    ...
    expect(find.text('Credenciais inválidas. Verifique o e-mail e a palavra-passe.'), findsOneWidget);
  });
}
