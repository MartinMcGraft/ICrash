...
Widget _wrap(AppServices services) {
  return AppServicesScope(
    services: services,
    child: MaterialApp(home: const ResponsibleUsersScreen(institutionId: 'inst-a', cartId: 'cart-1')),
  );
}

Membership _member(String uid, Role role) =>
    Membership(uid: uid, institutionId: 'inst-a', role: role, status: MembershipStatus.active);
...
void main() {
  ...
  testWidgets('lists members with a checkbox reflecting current responsibility', (tester) async {
    final carts = FakeCartRepository();
    carts.responsibleUsers.add(const CartResponsibleUser(uid: 'user-1', cartId: 'cart-1'));
    await tester.pumpWidget(_wrap(buildTestServices(
      institutions: FakeInstitutionRepository(members: [_member('user-1', Role.user), _member('user-2', Role.user)]),
      carts: carts,
    )));
    await tester.pumpAndSettle();

    final checkbox1 = tester.widget<CheckboxListTile>(
      find.ancestor(of: find.text('user-1'), matching: find.byType(CheckboxListTile)),
    );
    final checkbox2 = tester.widget<CheckboxListTile>(
      find.ancestor(of: find.text('user-2'), matching: find.byType(CheckboxListTile)),
    );
    expect(checkbox1.value, isTrue);
    expect(checkbox2.value, isFalse);
  });

  testWidgets('assigns and removes a responsible user by toggling the checkbox', (tester) async {
    final carts = FakeCartRepository();
    await tester.pumpWidget(_wrap(buildTestServices(
      institutions: FakeInstitutionRepository(members: [_member('user-1', Role.user)]),
      carts: carts,
    )));
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.byType(CheckboxListTile));
    await tester.pumpAndSettle();
    expect(carts.responsibleUsers.map((r) => r.uid), isNot(contains('user-1')));
  });
}
