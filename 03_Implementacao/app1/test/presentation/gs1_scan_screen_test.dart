...
void main() {
  testWidgets('parses a scanned GS1 payload and pops with the result', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: _HostScreen(scanner: scanner, onResult: (r) => result = r),
      ),
    );
  ...
  testWidgets(
    'shows a retry message and keeps listening when a payload has no recognized GS1 data',
    (tester) async {
      final scanner = FakeGs1DataMatrixScannerService();
      await tester.pumpWidget(
        MaterialApp(
          ...
          localizationsDelegates: AppLocalizations.localizationsDelegates,
          supportedLocales: AppLocalizations.supportedLocales,
          home: _HostScreen(scanner: scanner, onResult: (_) {}),
        ),
      );
      ...
      expect(
        find.text('Digitalizar código GS1'),
        findsOneWidget,
        reason: 'still on the scan screen, not popped',
      );
    },
  );

  testWidgets(
    'cancelling returns null so the caller falls back to manual entry',
    (tester) async {
      final scanner = FakeGs1DataMatrixScannerService();
      Object? result = 'unset';
      await tester.pumpWidget(
  ...
  testWidgets(
    'HID mode: typing a code into the focused field and pressing Enter parses and pops',
    (tester) async {
