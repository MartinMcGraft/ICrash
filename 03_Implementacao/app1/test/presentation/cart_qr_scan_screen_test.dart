    ...
    return Scaffold(
      body: Center(
        child: ElevatedButton(
          onPressed: () async {
            final result = await showCartQrScanScreen(
              context,
...
void main() {
  testWidgets('decodes a scanned cart QR payload and pops with the target', (
    tester,
  ) async {
  ...
  testWidgets('shows a retry message for an unrecognized payload', (
    tester,
  ) async {
    final scanner = FakeInternalQrScannerService();
    await tester.pumpWidget(
    ...
    expect(
      find.textContaining('não é um código de carro reconhecido'),
      findsOneWidget,
    );
    expect(
      find.text('Ler código do carro'),
      findsOneWidget,
