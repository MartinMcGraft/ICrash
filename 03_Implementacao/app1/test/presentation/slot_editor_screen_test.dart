...
Widget _wrap(AppServices services) {
  return AppServicesScope(
    ...
    child: MaterialApp(
      ...
      home: const SlotEditorScreen(cart: _cart, drawer: _drawer),
    ),
  );
}

Membership _manager() => const Membership(
  uid: 'me',
...
void main() {
  testWidgets(
    'renders one unit cell per grid position when no slots exist yet',
    (tester) async {
  ...
  testWidgets(
    'refuses to assign a product already assigned elsewhere in the same cart',
    (tester) async {
      const product = Product(
        id: 'p1',
        institutionId: 'inst-a',
      ...
      expect(
        find.textContaining('já está atribuído a outro slot'),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'replenishes stock by scanning a GS1 code, pre-filling lot/expiry and warning on unknown GTIN',
    (tester) async {
      const assignment = CartProductAssignment(
        id: 'assignment-1',
        cartId: 'cart-1',
      ...
      await tester.tap(
        find.widgetWithText(OutlinedButton, 'Digitalizar código GS1'),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.textContaining('GTIN não reconhecido no catálogo'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'hides manager-only actions but keeps consumption/correction for a normal user',
    (tester) async {
      const assignment = CartProductAssignment(
        id: 'assignment-1',
        cartId: 'cart-1',
  ...
  testWidgets('flags an orphaned assignment and lets a manager delete it', (
    tester,
  ) async {
    const product = Product(
      id: 'p1',
  ...
  testWidgets(
    'lets a manager reassign an orphaned assignment to a currently empty slot',
    (tester) async {
      const product = Product(
        id: 'p1',
        institutionId: 'inst-a',
  ...
  testWidgets(
    'scrolls instead of shrinking cells below the minimum touch target on a dense drawer',
    (tester) async {
      const denseDrawer = CartDrawer(
        id: 'drawer-1',
        cartId: 'cart-1',
        name: 'Gaveta densa',
        rows: 10,
      ...
      await tester.pumpWidget(
        AppServicesScope(
          ...
          child: MaterialApp(
            ...
            localizationsDelegates: AppLocalizations.localizationsDelegates,
            supportedLocales: AppLocalizations.supportedLocales,
            home: const SlotEditorScreen(cart: _cart, drawer: denseDrawer),
          ),
        ),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'a normal user long-pressing an empty slot only sees an informational message',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          buildTestServices(
      ...
      expect(
        find.textContaining('ainda não tem produto atribuído'),
        findsOneWidget,
      );
