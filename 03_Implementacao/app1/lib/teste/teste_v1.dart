...
class _TesteDataMatrixState extends State<TesteDataMatrix> {
  ...
  Future<String?> processImage(img.Image image) async {
    try {
      ...
      LuminanceSource source = RGBLuminanceSource(
        image.width,
        image.height,
        image.getBytes(order: img.ChannelOrder.bgra).buffer.asInt32List(),
      );
  ...
  void _extractGS1Data(String dataMatrix) {
    ...
    final regexSerialNumber = RegExp(
      r"21([A-Za-z0-9]{1,20})",
    ); // Alphanumeric Serial Number up to 20 characters
    final regexBatchNumber = RegExp(
      r"10([A-Za-z0-9]{1,20})",
    ); // Alphanumeric Batch Number up to 20 characters
    final regexExpirationDate = RegExp(
      r"17(\d{6})",
    ); // Expiration Date in YYMMDD format
    final regexRegistrationNumber = RegExp(
      r"714(\d{7})",
    ); // National Registration Number of 7 digits
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('DataMatrix Scanner')),
      body: Column(
        children: [
